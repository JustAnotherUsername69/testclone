...
def in_scr(update, context):
    ...
    if len(args) > 1:
        ...
        cmd = f"python3 web/index_scr.py {shlex.quote(link)}"
    process = Popen(cmd, stdout=PIPE, stderr=PIPE, shell=True)
    stdout, stderr = process.communicate()
    reply = ''
    stdout = stdout.decode()
...
def pahe(update, context):
    message = update.effective_message
    args = update.message.text.split(" ", maxsplit=1)
    link = ''
    if len(args) == 1:
        return message.reply_text('Send Pahe.li site link to Bypass ads and Get Last Link...', parse_mode='HTML')
    if len(args) > 1:
        link = args[1]
        msg = sendMessage("Bypassing Ads.! Please Wait...", context.bot, update.message)
        cmd = f"python3 web/pahe.py {shlex.quote(link)}"
    process = Popen(cmd, stdout=PIPE, stderr=PIPE, shell=True)
    stdout, stderr = process.communicate()
    reply = ''
    stdout = stdout.decode()
    ...
    if len(stdout) != 0:
        reply += f"{stdout}\n"
    if len(reply) > 3000:
        with open('pahe.txt', 'w') as file:
            file.write(reply)
        with open('pahe.txt', 'rb') as doc:
            context.bot.send_document(
                document=doc,
                filename=doc.name,
                reply_to_message_id=message.message_id,
        ...
        message.reply_text('Not a valid Pahe.li site Link. Check Again..', parse_mode='Markdown')

def psa(update, context):
    message = update.effective_message
    args = update.message.text.split(" ", maxsplit=1)
    link = ''
    if len(args) == 1:
        return message.reply_text('Send Psa.pm site link to Bypass ads and Get Last Link...', parse_mode='HTML')
    if len(args) > 1:
        link = args[1]
        msg = sendMessage("Bypassing Ads.! Please Wait...", context.bot, update.message)
        cmd = f"python3 web/psa.py {shlex.quote(link)}"
    process = Popen(cmd, stdout=PIPE, stderr=PIPE, shell=True)
    stdout, stderr = process.communicate()
    reply = ''
    stdout = stdout.decode()
    ...
    if len(stdout) != 0:
        reply += f"{stdout}\n"
    if len(reply) > 3000:
        with open('psa.txt', 'w') as file:
            file.write(reply)
        with open('psa.txt', 'rb') as doc:
            context.bot.send_document(
                document=doc,
                filename=doc.name,
                reply_to_message_id=message.message_id,
        ...
        message.reply_text('Not a valid Psa.pm site Link. Check Again..', parse_mode='Markdown')
...
G_HANDLER = CommandHandler(BotCommands.GCommand, g_lh,
                                                  filters=CustomFilters.authorized_chat | CustomFilters.authorized_user, run_async=True)
INSCR_HANDLER = CommandHandler(BotCommands.InScrCommand, in_scr,
                                                  filters=CustomFilters.owner_filter | CustomFilters.sudo_user, run_async=True)
PAHE_HANDLER = CommandHandler(BotCommands.PaheCommand, pahe,
                                                  filters=CustomFilters.owner_filter | CustomFilters.sudo_user, run_async=True)
PSA_HANDLER = CommandHandler(BotCommands.PsaCommand, psa,
                                                  filters=CustomFilters.owner_filter | CustomFilters.sudo_user, run_async=True)
...
dispatcher.add_handler(INSCR_HANDLER)
