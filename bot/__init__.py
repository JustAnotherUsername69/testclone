...
try:
    users = get_config('PAUTHORIZED_CHATS')
    users = users.split(" ")
    for user in users:
        PAUTHORIZED_CHATS.add(int(user))
except:
    pass

try:
    users = get_config('PRIVATE_CHATS')
    users = users.split(" ")
    for user in users:
        PRIVATE_CHATS.add(int(user))
except:
    pass
...
try:
    DRIVEHUB_CRYPT = get_config('DRIVEHUB_CRYPT')
